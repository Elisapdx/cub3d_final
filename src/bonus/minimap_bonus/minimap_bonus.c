...
void	load_img(t_config **conf)
{
	...
	mlx = (*conf)->mlx;
	(*conf)->floor = ft_new_sprite(mlx->mlx, \
		"./src/bonus/minimap_bonus/floor.xpm");
	(*conf)->door_c = ft_new_sprite(mlx->mlx, \
		"./src/bonus/minimap_bonus/door_close.xpm");
	(*conf)->door_o = ft_new_sprite(mlx->mlx, \
		"./src/bonus/minimap_bonus/door_open.xpm");
	(*conf)->wall = ft_new_sprite(mlx->mlx, \
		"./src/bonus/minimap_bonus/wall.xpm");
	(*conf)->tex_player = ft_new_sprite(mlx->mlx, \
		"./src/bonus/minimap_bonus/player.xpm");
	(*conf)->empty = ft_new_sprite(mlx->mlx, \
...
void	put_image_minmap(t_config **conf, t_vars *vars, int i, int j)
{
	if (((*conf)->pos_mmap_y >= 0 && (*conf)->pos_mmap_x < (*conf)->map_len)
		&& ((*conf)->pos_mmap_x >= 0 && (*conf)->pos_mmap_y < \
			(int)ft_strlen((*conf)->map[(int)(*conf)->pos_mmap_x])))
	{
		if ((*conf)->map[(int)(*conf)->pos_mmap_x] \
			[(int)(*conf)->pos_mmap_y] == '1')
			put_img_to_window(vars, (*conf)->wall, i * 12, j * 12);
		else if ((*conf)->map[(int)(*conf)->pos_mmap_x] \
			[(int)(*conf)->pos_mmap_y] == '0')
			put_img_to_window(vars, (*conf)->floor, i * 12, j * 12);
		else if ((*conf)->map[(int)(*conf)->pos_mmap_x] \
			[(int)(*conf)->pos_mmap_y] == '2')
			put_img_to_window(vars, (*conf)->door_c, i * 12, j * 12);
		else if ((*conf)->map[(int)(*conf)->pos_mmap_x] \
			[(int)(*conf)->pos_mmap_y] == '3')
			put_img_to_window(vars, (*conf)->door_o, i * 12, j * 12);
		else if (ft_strchr("NSWE", (*conf)->map[(int)(*conf)->pos_mmap_x] \
			[(int)(*conf)->pos_mmap_y]))
			put_img_to_window(vars, (*conf)->floor, i * 12, j * 12);
		else if ((*conf)->map[(int)(*conf)->pos_mmap_x] \
			[(int)(*conf)->pos_mmap_y] == ' ')
...
int	load_map(t_config **conf, t_vars *vars)
{
	...
	put_img_to_window(vars, (*conf)->tex_player, \
