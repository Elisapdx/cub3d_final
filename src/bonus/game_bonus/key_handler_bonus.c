...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/31 15:58:12 by epraduro          #+#    #+#             */
/*   Updated: 2023/12/31 18:36:37 by epraduro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	do_move(t_config **conf, double new_x, double new_y)
{
	...
	if (map[(int)(p->pos_x + new_x * (MOVE_SPEED + 0.1))]
		[(int)(p->pos_y)] != '1' && map[(int)(p->pos_x + new_x *
			(MOVE_SPEED + 0.1))][(int)(p->pos_y)] != '2')
		p->pos_x += new_x * MOVE_SPEED;
	if (map[(int)(p->pos_x)][(int)(p->pos_y + new_y * (MOVE_SPEED + 0.1))]
		!= '1' && map[(int)(p->pos_x)][(int)(p->pos_y + new_y *
			(MOVE_SPEED + 0.1))] != '2')
...
int	key_handler(int key_pressed, t_config **conf)
{
	if (key_pressed == ESC_KEY)
		ft_close(conf);
	if (key_pressed == 46 && !(*conf)->data->mouse_show)
	{
		(*conf)->data->mouse_show = true;
		return (0);
	}
	else if (key_pressed == 46 && (*conf)->data->mouse_show)
	{
